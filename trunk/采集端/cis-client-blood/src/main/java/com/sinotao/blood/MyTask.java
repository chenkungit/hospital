...
public class MyTask {
	...
	private void upload(){
		...
		while(this.data2upload.size()>0){
			String taskId = "blood_" + DateUtil.getNowByFormat("yyyyMMddHHmmss");
			...
			String checkNumber = m1.getOBR().getObr3_FillerOrderNumber().getEi1_EntityIdentifier().getValue();
			...
			for(int i=0; i<m1.getOBXNTEReps(); i++){
				...
				String itemDetailCode = obx.getObx3_ObservationIdentifier().getCe1_Identifier().getValue();
				String itemDetailName = obx.getObx3_ObservationIdentifier().getCe2_Text().getValue();
				...
				ccr.setItemDetailName(itemDetailName);
			...
			this.clinicarCheckResultBiz.insertBloodData(checkNumber, msg.toString(), ccrList);
