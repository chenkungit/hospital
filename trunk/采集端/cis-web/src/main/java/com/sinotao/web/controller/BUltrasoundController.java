 ...
 */ 
@Controller
@RequestMapping("/bUltra/") 
public class BUltrasoundController extends BaseDeviceController{

	private final static Logger logger = LoggerFactory.getLogger(BUltrasoundController.class);
	...
	private DptEnum dpt = DptEnum.BUTRA;
    ...
    public ResultDTO receiveData(HttpServletRequest request, String code, MultipartFile attachment){
    	...
    	String reqId = "BUtra-"+DateUtil.getNowByFormat("yyyyMMddHHmmss");
    	logger.info("收到请求"+reqId);
    	logger.info("请求"+reqId+"：检查号="+code);
		try {
			if(null!=attachment && attachment.getBytes().length>0){
				...
				String attachmentPath = UploadUtil.saveFile(attachment, this.uploadConfig.getBasePath(),
						this.uploadConfig.getbUtraFolder() + sourceFolder, 0L, null);
				...
				if(count > 0){
					...
					// 3.解压
					String descDir = zipAttachment(attachmentPath, this.uploadConfig.getbUtraFolder());
