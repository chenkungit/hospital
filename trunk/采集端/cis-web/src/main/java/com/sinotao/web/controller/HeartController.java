...
public class HeartController extends BaseDeviceController{
	...
	private DptEnum dpt = DptEnum.HEART;
    ...
    public ResultDTO receiveData(HttpServletRequest request, String code, MultipartFile attachment){
    	...
    	String reqId = "heart-"+DateUtil.getNowByFormat("yyyyMMddHHmmss");
		...
		try {
			if(null!=attachment && attachment.getBytes().length>0){
				...
				String attachmentPath = UploadUtil.saveFile(attachment, this.uploadConfig.getBasePath(),
						this.uploadConfig.getHeartFolder() + sourceFolder, 0L, null);
				...
				if(count > 0){
					...
					// 3.解压
					String descDir = zipAttachment(attachmentPath, this.uploadConfig.getHeartFolder());
