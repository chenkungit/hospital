...
@Controller
@RequestMapping("/xRay/") 
public class XRayController extends BaseDeviceController{
	
	private final static Logger logger= LoggerFactory.getLogger(XRayController.class);
	...
	private DptEnum dpt = DptEnum.XRAY;
    ...
    public ResultDTO receiveData(HttpServletRequest request, String code, MultipartFile attachment){
    	...
    	String reqId = "xRay-"+DateUtil.getNowByFormat("yyyyMMddHHmmss");
		...
		try {
			if(null!=attachment && attachment.getBytes().length>0){
				...
				String attachmentPath = UploadUtil.saveFile(attachment, this.uploadConfig.getBasePath(),
						this.uploadConfig.getxRayFolder() + sourceFolder, 0L, null);
				...
				if(count > 0){
					...
					// 3.解压
					String descDir = zipAttachment(attachmentPath, this.uploadConfig.getxRayFolder());
