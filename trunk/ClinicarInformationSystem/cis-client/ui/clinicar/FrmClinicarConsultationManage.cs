...
namespace cis_client.ui.clinicar
{
    public partial class FrmClinicarConsultationManage : SCommon.SControl.SBaseForm
    {
        ...
        private void dataUpload(SResult rst)
        {
            ...
            #region + 检查登记表
            DataTable _dtDj = MySQLHelper.ExecuteDataTable(SqlUtil.F_OutsideConnectionString, CommandType.Text, "select id from t_clinicar_check where deleted = 0 and check_number = '" + ((ClinicarConsultation)rst.data).CheckNumber + "'");
            ClinicarCheck clinicarCheck = clinicarCheckBiz.FindByCheckNum(((ClinicarConsultation)rst.data).CheckNumber);
            if (_dtDj == null || Convert.IsDBNull(_dtDj) || _dtDj.Rows.Count <= 0)
            {
                if (clinicarCheck != null)
                {
                    try
                    {
                        string sql = "insert into t_clinicar_check values("
                            + "'" + clinicarCheck.Id + "',"
                            + clinicarCheck.Deleted + ","
                            + "'" + clinicarCheck.Name + "',"
                            + "'" + clinicarCheck.GenderCode + "',"
                            + "'" + clinicarCheck.GenderName + "',"
                            + "'" + clinicarCheck.NationalityCode + "',"
                            + "'" + clinicarCheck.NationalityName + "',"
                            + "'" + clinicarCheck.Birthday + "',"
                            + "'" + clinicarCheck.Age + "',"
                            + "'" + clinicarCheck.MaritalStatusCode + "',"
                            + "'" + clinicarCheck.MaritalStatusName + "',"
                            + "'" + clinicarCheck.CertificateTypeCode + "',"
                            + "'" + clinicarCheck.CertificateTypeName + "',"
                            + "'" + clinicarCheck.CertificateNumber + "',"
                            + "'" + clinicarCheck.Address + "',"
                            + "'" + clinicarCheck.Tel + "',"
                            + "'" + clinicarCheck.Remark + "',"
                            + "'" + clinicarCheck.CheckNumber + "',"
                            + "'" + clinicarCheck.CheckDoctor + "',"
                            + "'" + clinicarCheck.CheckDate + "',"
                            + "'" + clinicarCheck.Conclusion + "',"
                            + "'" + clinicarCheck.Advice + "',"
                            + clinicarCheck.AllcheckCompleted + ","
                            + "'" + clinicarCheck.Disease + "'"
                            + ")";
                        MySQLHelper.ExecuteNonQuery(SqlUtil.F_OutsideConnectionString, CommandType.Text, sql);
                    }
                    catch (Exception exception)
                    {
                        UcMessageBox.Error("检查登记表数据上传失败 -- ", exception.Message + "\r\n" + exception.StackTrace + "\r\n" + exception.Source);
                    }
                }
            }
            #endregion
            #region + 检查项目表
            DataTable _dtXm = MySQLHelper.ExecuteDataTable(SqlUtil.F_OutsideConnectionString, CommandType.Text, "select id from t_clinicar_check_item where deleted = 0 and check_id = " + clinicarCheck.Id);
            if (_dtXm == null || Convert.IsDBNull(_dtXm) || _dtXm.Rows.Count <= 0)
            {
                System.Collections.IList list = clinicarCheckItemBiz.FindByCheckId(Convert.ToInt32(clinicarCheck.Id));
                if (list != null && list.Count > 0)
                {
                    try
                    {
                        ...
                        {
                            string sql = "insert into t_clinicar_check_item values("
                            + "'" + ((System.Collections.IList)list[i])[0] + "',"
                            + ((System.Collections.IList)list[i])[1] + ","
                            + "'" + ((System.Collections.IList)list[i])[2] + "',"
                            + "'" + ((System.Collections.IList)list[i])[3] + "',"
                            + "'" + ((System.Collections.IList)list[i])[4] + "',"
                            + "'" + ((System.Collections.IList)list[i])[5] + "',"
                            + "'" + ((System.Collections.IList)list[i])[6] + "',"
                            + ((System.Collections.IList)list[i])[7] + ","
                            + ((System.Collections.IList)list[i])[8] + ","
                            + "'" + ((System.Collections.IList)list[i])[9] + "',"
                            + "'" + ((System.Collections.IList)list[i])[10] + "',"
                            + "'" + ((System.Collections.IList)list[i])[11] + "'"
                            + ")";
                            MySQLHelper.ExecuteNonQuery(SqlUtil.F_OutsideConnectionString, CommandType.Text, sql);
                        }
                    ...
                    {
                        UcMessageBox.Error("检查项目数据上传失败 -- ", exception.Message + "\r\n" + exception.StackTrace + "\r\n" + exception.Source);
                    }
                }
            }
            #endregion
                    ...
                    try
                    {
                        for (int i = 0; i < _dt.Rows.Count; i++)
                        {
                            string sql = "insert into t_clinicar_check_result values ("
                                       ...
                                       + "'" + _dt.Rows[i][13] + "',"
                                       + "'" + _dt.Rows[i][14] + "',"
                                       + "'" + _dt.Rows[i][15].ToString().Replace(@"\", @"\\") + "'"
                                       + " )";

                            this.uploadFile(_dt.Rows[i][11].ToString(), _dt.Rows[i][15].ToString(), _dt.Rows[i][3].ToString());     //上传文件
                            MySQLHelper.ExecuteNonQuery(SqlUtil.F_OutsideConnectionString, CommandType.Text, sql);
                        }
                        //上传B超Logo
                        this.uploadLogoFile();
        ...
        private void uploadLogoFile()
        {
            ...
            ftpUristring += ":21";
            //报告LOGO图像同步
            string logoPath = strLocalFilePath + @"bc\";
            DirectoryInfo logoDir = new DirectoryInfo(logoPath);
            if (logoDir.Exists)
            {
                FileInfo[] files = logoDir.GetFiles("ReportLOGO.bmp");
                if (files != null && files.Length > 0)
                {
                    try
                    {
                        string uri = GetUriString(@"bc/" + files[0].Name);
                        //检查目录是否存在，不存在创建
                        FtpCheckDirectoryExist(@"bc/" + files[0].Name);
        ...
        /// relativeReportPath : 报告的相对路径
        /// </summary>
        private void uploadFile(string relativePath, string relativeReportPath, string checkNumber)
        {
            ...
            if (!string.IsNullOrEmpty(relativePath))
            {
                ...
                if (!string.IsNullOrEmpty(bdPath))
                {
                    try
                    {
                        DirectoryInfo bdDir = new DirectoryInfo(bdPath);

                        if (bdDir.Exists)
                        {
                            FileInfo[] files = bdDir.GetFiles(fileName);
                            if (files != null && files.Length > 0)
                            {
                                try
                                {
                                    string uri = GetUriString(relativePath);
                                    //检查目录是否存在，不存在创建
            ...
            //上传报告附件
            if (!string.IsNullOrEmpty(relativeReportPath))
            {
                string strFilePath = strLocalFilePath;          //需要上传的本地文件固定路径前缀
                strFilePath = strFilePath + relativeReportPath;           //拼接路径
                if (!File.Exists(strFilePath)) return;
                string[] x = strFilePath.Split('\\');
                string bdPath = string.Empty;
                string fileName = x[x.Length - 1];
                ...
                if (!string.IsNullOrEmpty(bdPath))
                {
                    try
                    {
                        ...
                        if (bdDir.Exists)
                        {
                            ...
                            if (files != null && files.Length > 0)
                            {
                                try
                                {
                                    string uri = GetUriString(relativeReportPath);
                                    //检查目录是否存在，不存在创建
                                    FtpCheckDirectoryExist(relativePath);
                                    FtpWebRequest request = CreateFtpWebRequest(uri, WebRequestMethods.Ftp.UploadFile);
                                    request.ContentLength = files[0].Length;
                                ...
                                {
                                    UcMessageBox.Error("上传发生错误，返回信息为：" + ex.Message, "提示");
                                }
                            }
                        }
                    }
                    ...
                    {
                        UcMessageBox.Error("上传发生错误，返回信息为：" + e1.Message, "提示");
                    }
                }
            }
            //B超报告同步
            ...
            //x光报告与图片同步
            //string xPath = strLocalFilePath + @"xRayFolder\data\" + checkNumber + @"\";
            //DirectoryInfo xDir = new DirectoryInfo(xPath);
            //if (xDir.Exists)
            //{
            //    FileInfo[] files = xDir.GetFiles("*.png");
            //    if (files != null && files.Length > 0)
            //    {
            //        try
            //        {
            ...
            //            {
            //                string uri = GetUriString(@"xRayFolder\data\" + checkNumber + @"\" + files[f].Name);
            //                //检查目录是否存在，不存在创建
            //                FtpCheckDirectoryExist(@"xRayFolder\data\" + checkNumber + @"\" + files[f].Name);
            //                FtpWebRequest request = CreateFtpWebRequest(uri, WebRequestMethods.Ftp.UploadFile);
            //                request.ContentLength = files[f].Length;
            //                int buflength = 8196;
            //                byte[] buffer = new byte[buflength];
            //                FileStream filestream = files[f].OpenRead();
            //                Stream responseStream = request.GetRequestStream();
            //                //lstbxFtpState.Items.Add("打开上传流，文件上传中...");
            //                int contenlength = filestream.Read(buffer, 0, buflength);
            //                while (contenlength != 0)
            ...
            //    FileInfo[] bmpFiles = xDir.GetFiles("*.bmp");
            //    if (bmpFiles != null && bmpFiles.Length > 0)
            //    {
            //        try
            //        {
            //            for (int f = 0; f < bmpFiles.Length; f++)
            //            {
            //                string uri = GetUriString(@"xRayFolder\data\" + checkNumber + @"\" + bmpFiles[f].Name);
            //                //检查目录是否存在，不存在创建
            //                FtpCheckDirectoryExist(@"xRayFolder\data\" + checkNumber + @"\" + bmpFiles[f].Name);
            //                FtpWebRequest request = CreateFtpWebRequest(uri, WebRequestMethods.Ftp.UploadFile);
            //                request.ContentLength = bmpFiles[f].Length;
            //                int buflength = 8196;
            //                byte[] buffer = new byte[buflength];
            //                FileStream filestream = bmpFiles[f].OpenRead();
            //                Stream responseStream = request.GetRequestStream();
            //                //lstbxFtpState.Items.Add("打开上传流，文件上传中...");
            //                int contenlength = filestream.Read(buffer, 0, buflength);
            //                while (contenlength != 0)
            ...
            //阴道镜文件同步
            //string ydjPath = strLocalFilePath + @"ydj\data\" + checkNumber + @"\";
            //DirectoryInfo ydjDir = new DirectoryInfo(ydjPath);
            //if (ydjDir.Exists)
            //{
            //    FileInfo[] files = ydjDir.GetFiles("*.pdf");
            //    if (files != null && files.Length > 0)
            //    {
            //        try
            //        {
            //            string uri = GetUriString(@"ydj\data\" + checkNumber + @"\" + files[0].Name);
            //            //检查目录是否存在，不存在创建
            //            FtpCheckDirectoryExist(@"ydj\data\" + checkNumber + @"\" + files[0].Name);
            //            FtpWebRequest request = CreateFtpWebRequest(uri, WebRequestMethods.Ftp.UploadFile);
            //            request.ContentLength = files[0].Length;
            //            int buflength = 8196;
            //            byte[] buffer = new byte[buflength];
