...
namespace cis_client.ui.clinicar
{
    public partial class FrmClinicarConsultation : SCommon.SControl.SBaseDockForm
    {
        ...
        private void uTxt_deptCode_DoubleClick(object sender, EventArgs e)
        {
            ...
            {
                FrmClinihospitalDptSearch frmClinicarDpt = new FrmClinihospitalDptSearch();
        ...
        private void createDate(string type)
        {
            ...
            if (!string.IsNullOrEmpty(uTxt_deptCode.Text))
            {
                ...
                //从服务器外网查询医院信息
                DataTable _dtOutSide = MySQLHelper.ExecuteDataTable(SqlUtil.F_OutsideConnectionString, CommandType.Text, "select id,sechedulDate,sechedulType,deptCode from t_clinicar_dpt_sechedul where sechedulType='上午' and deptCode='" + uTxt_deptCode.Text + "'  and sechedulDate>='" + lbl_datefrom.Text + "'  and sechedulDate<='" + lbl_dateto.Text + "'");

                if (_dtOutSide != null && _dtOutSide.Rows.Count > 0)
                {
                    foreach (DataRow cds in _dtOutSide.Rows)
                    {
                ...
                //从服务器外网查询医院信息
                DataTable _dtOutSideXw = MySQLHelper.ExecuteDataTable(SqlUtil.F_OutsideConnectionString, CommandType.Text, "select id,sechedulDate,sechedulType,deptCode from t_clinicar_dpt_sechedul where sechedulType='下午' and deptCode='" + uTxt_deptCode.Text + "'  and sechedulDate>='" + lbl_datefrom.Text + "'  and sechedulDate<='" + lbl_dateto.Text + "'");
                if (_dtOutSideXw != null && _dtOutSideXw.Rows.Count > 0)
                {
                    foreach (DataRow cds in _dtOutSideXw.Rows)
                    {
                        string weekStr = Convert.ToDateTime(cds["sechedulDate"]).DayOfWeek.ToString();
                        switch (weekStr)
                        {
        ...
        {
            FrmCliniHospital frmClinicarHospital = new FrmCliniHospital();
        ...
        private void setLinkText(LinkLabel linklab,Label lbl_d)
        {
            if ("可申请".Equals(linklab.Text))
            {
                ...
                if (linklab.Name.Contains("lbl_sw"))
                    cc.ConsultationType = "上午";
                else
                    cc.ConsultationType = "下午";
