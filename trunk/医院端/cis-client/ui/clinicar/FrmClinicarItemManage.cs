...
namespace cis_client.ui.clinicar
{
    ...
    {
        private ClinicarItemBiz clinicarDptBiz = new ClinicarItemBiz();
        ...
        private bool CheckBeforeSave()
        {
            ...
            if (uchk_enabled.Checked && uCbo_DeviceCode.SelectedValue!=null && !uCbo_DeviceCode.SelectedValue.ToString().Equals(""))
            {
                ...
                if (this.DialogStatus == UBaseLib.Enums.DialogStatus.New) //新增
                {
                    bool isExist = clinicarDptBiz.CheckIsExistForDeviceCode(uCbo_DeviceCode.SelectedValue.ToString(),"");
                    if (isExist)
                    {
                ...
                {
                    bool isExist = clinicarDptBiz.CheckIsExistForDeviceCode(uCbo_DeviceCode.SelectedValue.ToString(), uTxt_code.Text.Trim());
                    if (isExist)
                    {
                        UcMessageBox.Warning("该设备已被使用！", "设备编号重复");
                        this.uCbo_DeviceCode.Focus();
