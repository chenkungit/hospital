    ...
    public partial class FrmClinicarAllCheck : SCommon.SControl.SBaseDockForm
    {
        ...
        private void queryCheckNumber()
        {
            ...
            if (!string.IsNullOrEmpty(ucTxt_code.Text.Trim()))
            {
                ...
                {
                    //数据类展示(表格)
                    if (Util.showType(ccr.ItemName).Equals(Util.data))
                    {
                        dataList.Add(ccr);
                    }
                    else if (Util.showType(ccr.ItemName).Equals(Util.image))
                    {
                        picList.Add(ccr);
                    }
                    else if (Util.showType(ccr.ItemName).Equals(Util.pdf))
                    {
                        pdfList.Add(ccr);
                    }
                }
                ...
                if (pdfList != null && pdfList.Count > 0)
                {
                    for (int i = 0; i < pdfList.Count; i++)
                    {
                        ...
                        if (pdfEntity != null)
                        {
                            if ("心电图".Equals(pdfEntity.ItemName.ToUpper()))
                            {
                                TabIndex = TabIndex + 1;
                            ...
                            else if ("B超".Equals(pdfEntity.ItemName.ToUpper()))
                            {
                                TabIndex = TabIndex + 1;
                    ...
                    for (int i = 0; i < completedItemList.Count; i++)
                    {
                        if (i == 0)
                        {
                            if (completedItemList[i].Summary != null && !string.IsNullOrEmpty(completedItemList[i].Summary.Trim()))
                                _disease = (i + 1) + ". " + completedItemList[i].Summary;
                        ...
                        {
                            if (completedItemList[i].Summary != null && !string.IsNullOrEmpty(completedItemList[i].Summary.Trim()))
                                _disease = _disease + "\r\n" + (i + 1) + ". " + completedItemList[i].Summary;
