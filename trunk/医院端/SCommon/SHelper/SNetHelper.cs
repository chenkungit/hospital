...
namespace SCommon.SUtil
{
    ...
    public sealed class NetHelper
    {
        ...
        #region Socket连接请求超时机制

        public static TcpClient TryConnect(IPEndPoint remoteEndPoint, int timeoutMiliSecond)
        {
            return TryConnect(Convert.ToString(remoteEndPoint.Address), remoteEndPoint.Port, timeoutMiliSecond);
        }
        ...
        public static TcpClient TryConnect(string serverip, int serverport, int timeoutMiliSecond)
        {
            ...
            IAsyncResult asyncResult = tcpClient.BeginConnect(serverip, serverport,
                new AsyncCallback(delegate(IAsyncResult aResult)
                {
                    try
                    {
                        TcpClient tcp = aResult.AsyncState as TcpClient;
                        tcp.EndConnect(aResult);
                    }
                    catch (Exception ex)
                    {
                        socketexception = ex;
            ...
            if (asyncResult.AsyncWaitHandle.WaitOne(timeoutMiliSecond, false))
            {
